import { D as DEV, n as noop, s as safe_not_equal } from "./chunks/index.js";
import {
  a as assets,
  b as base,
...
async function handle_error_and_jsonify(event, options2, error) {
  ...
  return await options2.hooks.handleError({ error, event, status, message }) ??
    { message };
}
...
function uneval(value, replacer) {
  ...
  Array.from(counts).filter((entry) => entry[1] > 1).sort((a, b) => b[1] - a[1])
    .forEach((entry, i) => {
  ...
  function stringify2(thing) {
    ...
    switch (type) {
      ...
      default:
        const obj = `{${
          Object.keys(thing).map((key2) =>
            `${safe_key(key2)}:${stringify2(thing[key2])}`
          ).join(",")
        }}`;
        const proto = Object.getPrototypeOf(thing);
        if (proto === null) {
    ...
    names.forEach((name, thing) => {
      ...
      switch (type) {
        ...
        case "Set":
          ...
          statements.push(
            `${name}.${
              Array.from(thing).map((v) => `add(${stringify2(v)})`).join(".")
            }`,
          );
          break;
        case "Map":
          ...
          statements.push(
            `${name}.${
              Array.from(thing).map(([k, v]) =>
                `set(${stringify2(k)}, ${stringify2(v)})`
              ).join(".")
            }`,
          );
          break;
...
async function call_action(event, actions) {
  ...
  if (!is_form_content_type(event.request)) {
    throw new SvelteKitError(
      ...
      `Form actions expect form-encoded data — received ${
...
function try_deserialize(data, fn, route_id) {
  try {
    ...
    if ("path" in error) {
      let message =
        `Data returned from action inside ${route_id} is not serializable: ${error.message}`;
...
function create_universal_fetch(event, state, fetched, csr, resolve_opts) {
  const universal_fetch = async (input, init2) => {
    const cloned_body = input instanceof Request && input.body
      ? input.clone().body
      : null;
    const cloned_headers = input instanceof Request && [...input.headers].length
      ...
      get(response2, key2, _receiver) {
        async function push_fetched(body2, is_b64) {
          ...
          if (isNaN(status_number)) {
            throw new Error(
              `response.status is not a number. value: "${response2.status}" type: ${typeof response2
                .status}`,
            );
          }
          fetched.push({
...
function serialize_data(fetched, filter, prerendering2 = false) {
  ...
  if (!prerendering2 && fetched.method === "GET" && cache_control && !varyAny) {
    const match = /s-maxage=(\d+)/g.exec(cache_control) ??
...
function sha256(data) {
  ...
  for (let i = 0; i < array2.length; i += 16) {
    ...
    for (let i2 = 0; i2 < 64; i2++) {
      ...
      tmp = tmp + out7 +
        (out4 >>> 6 ^ out4 >>> 11 ^ out4 >>> 25 ^ out4 << 26 ^ out4 << 21 ^
          out4 << 7) +
        (out6 ^ out4 & (out5 ^ out6)) + key[i2];
      out7 = out6;
      out6 = out5;
      out5 = out4;
      out4 = out3 + tmp | 0;
      ...
      out2 = out1;
      out1 = out0;
      out0 = tmp + (out1 & out2 ^ out3 & (out1 ^ out2)) +
          (out1 >>> 2 ^ out1 >>> 13 ^ out1 >>> 22 ^ out1 << 30 ^ out1 << 19 ^
            out1 << 10) | 0;
    }
    out[0] = out[0] + out0 | 0;
    out[1] = out[1] + out1 | 0;
...
class BaseProvider {
  ...
  get_header(is_meta = false) {
    ...
    for (const key2 in directives) {
      if (
        is_meta &&
        (key2 === "frame-ancestors" || key2 === "report-uri" ||
          key2 === "sandbox")
      ) {
        continue;
      }
...
async function render_response({
  ...
  if (page_config.csr) {
    ...
    const included_modulepreloads = Array.from(
      modulepreloads,
      (dep) => prefixed(dep),
    ).filter(
    ...
    const blocks = [];
    const load_env_eagerly = client.uses_env_dynamic_public &&
      state.prerendering;
...
${indent}}`);
    ...
    if (load_env_eagerly) {
      blocks.push(
        `import(${
          s(`${base$1}/${options2.app_dir}/env.js`)
        }).then(({ env }) => {
						${global}.env = env;
...
`);
  ...
  try {
    const strings = nodes.map((node) => {
      if (!node) return "null";
      return `{"type":"data","data":${uneval(node.data, replacer)},${
        stringify_uses(node)
...
`);
  ...
  try {
    const strings = nodes.map((node) => {
      ...
      return `{"type":"data","data":${stringify(node.data, reducers)},${
        stringify_uses(
          node,
        )
      }${node.slash ? `,"slash":${JSON.stringify(node.slash)}` : ""}}`;
...
async function render_page(
  ...
  try {
    ...
    if (
      ...
      if (
        DEV && action_result && !event.request.headers.has("x-sveltekit-action")
      );
      return await render_response({
        branch: [],
        fetched,
...
function exec(match, params, matchers) {
  ...
  for (let i = 0; i < params.length; i += 1) {
    ...
    if (!param.matcher || matchers[param.matcher](value)) {
      ...
      if (
        next_param && !next_param.rest && next_param.optional && next_value &&
        param.chained
      ) {
        buffered = 0;
      }
      if (
        !next_param && !next_value &&
...
function get_cookies(request, url, trailing_slash) {
  ...
  const cookies = {
    ...
    get(name, opts) {
      ...
      if (
        c && domain_matches(url.hostname, c.options.domain) &&
  ...
  function get_cookie_header(destination, header2) {
    ...
    return Object.entries(combined_cookies).map(([name, value]) =>
      `${name}=${value}`
    ).join("; ");
  }
  ...
  try {
    ...
    console.error(
      "set-cookie-parser encountered an error while decoding a cookie with value '" +
        value + "'. Set options.decodeValues to false to disable this feature.",
      e,
    );
  }
  var cookie = {
    ...
    return options2.hooks.handleFetch({
      ...
      fetch: async (info2, init3) => {
        ...
        if (info2 !== original_request) {
          mode = (info2 instanceof Request ? info2.mode : init3?.mode) ??
            "cors";
          credentials = (info2 instanceof Request
            ? info2.credentials
...
async function respond(request, options2, manifest, state) {
  ...
  if (options2.csrf_check_origin) {
    const forbidden = is_form_content_type(request) &&
  ...
  let rerouted_path;
  try {
    rerouted_path = options2.hooks.reroute({ url: new URL(url) }) ??
      url.pathname;
  } catch {
  ...
  if (is_data_request) {
    ...
    invalidated_data_nodes = url.searchParams.get(INVALIDATED_PARAM)?.split("")
      .map((node) => node === "1");
    ...
    try {
      ...
      if (route) {
        ...
        if (is_data_request) {
          ...
          route.endpoint && (!route.page || is_endpoint_request(event2))
        ) {
        ...
        if (request.method === "GET" && route.page && route.endpoint) {
          const vary = response.headers.get("vary")?.split(",")?.map((v) =>
            v.trim().toLowerCase()
          );
